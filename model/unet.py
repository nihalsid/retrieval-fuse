...
def create_conv(in_channels, out_channels, kernel_size, order, num_groups, padding=1):
    ...
    :param num_groups: number of groups for the GroupNorm
    ...
    :param kernel_size: add zero-padding to the input
...
class StepDownDoubleConv(nn.Sequential):

    def __init__(self, in_channels, out_channels, encoder, kernel_size=3, order='crg', num_groups=8):
        # conv1
        super().__init__()
        self.encoder = encoder
        conv1_in_channels, conv1_out_channels = in_channels, (in_channels + out_channels) // 2
        conv2_in_channels, conv2_out_channels = (in_channels + out_channels) // 2, out_channels
        self.add_module('SingleConv1',
                        SingleConv(conv1_in_channels, conv1_out_channels, kernel_size, order, num_groups))
        # conv2
        self.add_module('SingleConv2',
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
        ...
        if basic_module == DoubleConv or basic_module == StepDownDoubleConv:
...
class DecoderNoJoining(Decoder):

    def __init__(self, in_channels, out_channels, kernel_size=3, scale_factor=(2, 2, 2), basic_module=DoubleConv,
                 conv_layer_order='crg', num_groups=8, mode='nearest'):
        super(DecoderNoJoining, self).__init__(in_channels, out_channels, kernel_size, scale_factor, basic_module,
                                               conv_layer_order, num_groups, mode)

    # noinspection PyMethodOverriding
    def forward(self, x):
        x = self.upsampling(encoder_features=torch.randn(1, 1, x.shape[2] * 2, x.shape[3] * 2, x.shape[4] * 2), x=x)
        x = self.basic_module(x)
        return x
...
class Abstract3DUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='gcr',
                 num_groups=8, num_levels=4, remove_n_final_layers=0, is_segmentation=False, final_conv=False, testing=False, **kwargs):
        ...
        if remove_n_final_layers > 0:
            reversed_f_maps = reversed_f_maps[:-remove_n_final_layers]
        reversed_f_maps_modified = list(reversed_f_maps)
        if not final_conv:
            reversed_f_maps_modified[-1] = out_channels
        for i in range(len(reversed_f_maps) - 1):
            ...
            out_feature_num = reversed_f_maps_modified[i + 1]
            # TODO: if non-standard pooling was used, make sure to use correct striding for transpose conv
            # currently strides with a constant stride: (2, 2, 2)
            if i == (len(reversed_f_maps) - 2) and not final_conv and remove_n_final_layers > 0:
                decoder = Decoder(in_feature_num, out_feature_num, basic_module=StepDownDoubleConv,
                                  conv_layer_order=layer_order, num_groups=num_groups)
            else:
...
class UNet3D(Abstract3DUNet):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=True, f_maps=64, layer_order='gcr',
                 num_groups=8, num_levels=4, is_segmentation=True, remove_n_final_layers=0, final_conv=False, **kwargs):
        super(UNet3D, self).__init__(in_channels=in_channels, out_channels=out_channels, final_sigmoid=final_sigmoid,
                                     ...
                                     num_groups=num_groups, num_levels=num_levels, is_segmentation=is_segmentation, final_conv=final_conv, remove_n_final_layers=remove_n_final_layers,
                                     **kwargs)
